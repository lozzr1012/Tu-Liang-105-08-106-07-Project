...
public class CopyOfMR_FPTree {

    private static final int minSuport = 0; // ����舀�摨�

    public static class GroupMapper extends
            Mapper<LongWritable, Text, Text, Text> {

        LinkedHashMap<String, Integer> freq = new LinkedHashMap<String, Integer>(); // 憸�1憿寥�
        ...
        public void setup(Context context) throws IOException {
            ...
            // 隞DFS�辣霂餃憸�1憿寥�嚗霂餃�IMWordCount���箸�隞塚�閬�撌脩���憸�摨�憟�
            ...
            Path freqFile = new Path(fs.getWorkingDirectory().toString() + "/output/part-r-00000");
         ...
         //   if (arr.length == 4) {
                ...
                for (String ele : result) {
                    ...
                    if (freq.containsKey(ele)) {
                       // list.add(ele.toLowerCase()); // 憒���望�摮�嚗�蝏�頧祆銝箏���
                    	list.add(ele);
                    }
                }
...
//                        	  context.write(new Text(list.get(i)),
//                        			  new Text(sb.toString()));
//                }
    ...
    public static void main(String[] args) throws IOException,
        ...
        FileSystem fs = FileSystem.get(conf);
        System.out.println(fs.getWorkingDirectory().toString() + "/input");
        Path inpath = new Path(fs.getWorkingDirectory().toString() + "/input");
        Path outpath = new Path(fs.getWorkingDirectory().toString() + "/output2");
    //  fs.delete(outpath, true);

        Job FPTreejob = new Job(conf, "MR_FPTree");
        FPTreejob.setJarByClass(CopyOfMR_FPTree.class);
